...
exports.validateFoodAvailabilityForCart = exports.resolveBranchIdForCart = void 0;
const BadRequest_1 = require("../Errors/BadRequest");
const pricing_helper_1 = require("./pricing.helper");
const food_helper_1 = require("./food.helper");
/**
 ...
 */
const resolveBranchIdForCart = async (branchId, addressId, restaurantId) => {
    ...
    try {
        return await (0, pricing_helper_1.resolveBranchIdFromAddress)(addressId, restaurantId);
    }
    catch {
        return null;
    }
};
exports.resolveBranchIdForCart = resolveBranchIdForCart;
/**
 ...
 */
const validateFoodAvailabilityForCart = async (foodId, branchId, addressId, restaurantId) => {
    const targetBranchId = await (0, exports.resolveBranchIdForCart)(branchId, addressId, restaurantId);
    if (!targetBranchId)
        return;
    const unavailableMap = await (0, food_helper_1.getUnavailableBranchesForFoods)([foodId]);
    const unavailableBranches = unavailableMap.get(foodId) || [];
    const isUnavailable = unavailableBranches.some((b) => b.id === targetBranchId);
    if (isUnavailable) {
        if (branchId) {
            throw new BadRequest_1.BadRequest("This item is currently unavailable in the selected branch.");
        }
        if (addressId) {
            throw new BadRequest_1.BadRequest("This item is currently unavailable for delivery to your selected address.");
        }
        throw new BadRequest_1.BadRequest("This item is currently unavailable at your location.");
    }
};
exports.validateFoodAvailabilityForCart = validateFoodAvailabilityForCart;
