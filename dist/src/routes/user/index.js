"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const auth_1 = __importDefault(require("./auth"));
const order_1 = __importDefault(require("./order"));
const home_1 = __importDefault(require("./home"));
const profile_1 = __importDefault(require("./profile"));
const notification_1 = __importDefault(require("./notification"));
const cart_1 = __importDefault(require("./cart"));
const user_wallets_1 = __importDefault(require("./user_wallets"));
const address_1 = __importDefault(require("./address"));
const authenticated_1 = require("../../middlewares/authenticated");
const authorized_1 = require("../../middlewares/authorized");
const favlist_1 = __importDefault(require("./favlist"));
const restaurantFeatures_1 = __importDefault(require("./restaurantFeatures"));
const rating_1 = __importDefault(require("./rating"));
const SocialMedia_1 = __importDefault(require("./SocialMedia"));
const slider_1 = __importDefault(require("./slider"));
const image_1 = __importDefault(require("./image"));
const offers_1 = __importDefault(require("./offers"));
const landingPage_1 = __importDefault(require("./landingPage"));
const userPoints_1 = __importDefault(require("./userPoints"));
const restaurantSettings_1 = __importDefault(require("./restaurantSettings"));
const recommendedFood_1 = __importDefault(require("./recommendedFood"));
const policyRouter_1 = __importDefault(require("./policyRouter"));
const popup_1 = __importDefault(require("./popup"));
const coupon_1 = __importDefault(require("./coupon"));
const restaurantGroups_1 = __importDefault(require("./restaurantGroups"));
const bundles_1 = __importDefault(require("./bundles"));
const food_1 = __importDefault(require("./food"));
const router = (0, express_1.Router)();
router.use("/home", home_1.default);
router.use("/auth", auth_1.default);
router.use("/socialmedia", SocialMedia_1.default);
router.use("/slider", slider_1.default);
router.use("/image", image_1.default);
router.use("/rating", rating_1.default);
router.use("/landing-page", landingPage_1.default);
router.use("/settings", restaurantSettings_1.default);
router.use("/recommended-foods", recommendedFood_1.default);
router.use("/policy", policyRouter_1.default);
router.use("/popup", popup_1.default);
router.use("/offers", offers_1.default);
router.use('/bundles', bundles_1.default);
router.use("/restaurant-groups", restaurantGroups_1.default);
router.use("/foods", food_1.default);
router.use("/restaurants", restaurantFeatures_1.default);
router.use(authenticated_1.authenticated, (0, authorized_1.authorizeRoles)("user"));
router.use("/profile", profile_1.default);
router.use("/order", order_1.default);
router.use("/wallet", user_wallets_1.default);
router.use("/address", address_1.default);
router.use("/cart", cart_1.default);
router.use("/favlist", favlist_1.default);
router.use("/notifications", notification_1.default);
router.use("/points", userPoints_1.default);
router.use("/coupon", coupon_1.default);
exports.default = router;
