...
export const addToCart = async (req: Request | any, res: Response) => {
    ...
    if (!foodId && !offerId) {
        throw new BadRequest("foodId or offerId is required");
    }

    // ─── If adding a bundle offer ────────────────────────────────────
    if (offerId) {
        const [offer] = await db.select().from(offers).where(eq(offers.id, offerId)).limit(1);
        if (!offer) throw new BadRequest("Offer not found");
        if (offer.status !== "active") throw new BadRequest("This offer is no longer active");
        const now = new Date();
        if (new Date(offer.endDate) < now) throw new BadRequest("This offer has expired");
        if (new Date(offer.startDate) > now) throw new BadRequest("This offer has not started yet");

        await validateUserNotBlocked(userId, offer?.restaurantId || "");

        const bundleFoods = await db.select().from(offerFoods).where(eq(offerFoods.offerId, offerId));
        if (bundleFoods.length === 0) {
            throw new BadRequest("This bundle offer has no foods configured.");
        }
        const representativeFoodId: string = foodId || bundleFoods[0].foodId;

        let resolvedBranchId: string | null = branchId || null;
        if (!resolvedBranchId && addressId) {
            try {
                resolvedBranchId = await resolveBranchIdFromAddress(addressId, offer.restaurantId || "");
            } catch (err: any) {
        ...
        if (resolvedBranchId) {
            const [br] = await db
                ...
                .from(branches)
                .where(and(eq(branches.id, resolvedBranchId), eq(branches.restaurantId, offer?.restaurantId || ""), eq(branches.status, "active")))
                .limit(1);
        ...
        const [existingOfferItem] = await db.select().from(cartItems)
            .where(and(eq(cartItems.userId, userId), eq(cartItems.offerId, offerId)))
            .limit(1);
        ...
        if (existingOfferItem) {
            ...
            await db.update(cartItems)
                ...
                .where(eq(cartItems.id, existingOfferItem.id));
        } else {
...
export const getCart = async (req: Request | any, res: Response) => {
    ...
    const offerFoodsGroupMap = new Map<string, any[]>();
    ...
    const formattedOfferAvailableItems: any[] = [];
    ...
    const allFoodIds = normalItems.map(i => i.foodId).filter((id): id is string => id !== null && id !== undefined);
...
export const updateCartItem = async (req: Request | any, res: Response) => {
    ...
    if (cartItem.offerId) {
        const [offer] = await db.select().from(offers).where(eq(offers.id, cartItem.offerId)).limit(1);
        if (!offer) throw new BadRequest("Offer not found");
        if (offer.status !== "active") throw new BadRequest("Offer is no longer active");
...
export const validateCartPricing = async (req: Request | any, res: Response) => {
    ...
    for (const item of userCartItems) {
        if (item.offerId) {
            ...
            if (priceChanged) isPriceChanged = true;
